import { BOUNDARIES, ENGAGEMENTS, FAQS, PUBLIC_STEPS } from "@/lib/catalog";
import { LEGAL_ALLOWS, LEGAL_DOES_NOT, PRIVACY_SECTIONS, TERMS_EFFECTIVE, TERMS_SECTIONS, TERMS_VERSION, type LegalSection } from "@/lib/legal";
...
export type StatCopy = { k: string; v: string };
export type FaqCopy = { q: string; a: string };
export type StepCopy = { n: string; title: string; body: string };
export type EngagementCopy = { id: string; label: string; bestFor: string; examples: string };
...
export type LegalCopy = {
  ...
  termsDek: string;
  termsImage: string;
  termsImageAlt: string;
  privacyKicker: string;
  privacyTitle: string;
  privacyDek: string;
  privacyImage: string;
  privacyImageAlt: string;
  terms: LegalSection[];
...
export type SiteCopy = {
  ...
  services: HeroCopy;
  offers: { kicker: string; title: string; dek: string };
  packages: HeroCopy & { sectionTitle: string; sectionDek: string };
  work: HeroCopy & {
    pathTitle: string;
    engagementTitle: string;
    engagementImage: string;
    engagementImageAlt: string;
    boundariesTitle: string;
  };
  start: HeroCopy;
  receptionist: HeroCopy;
  login: LoginCopy;
  account: HeroCopy;
  nav: NavCopy;
  contact: ContactCopy;
  footer: { blurb: string; legalLine: string };
  cta: { title: string; body: string; startLabel: string; receptionLabel: string };
  faqs: FaqCopy[];
  steps: StepCopy[];
...
function hero(partial: HeroCopy): HeroCopy {
  return partial;
}

export function defaultSiteCopy(): SiteCopy {
  return {
    home: {
      ...
      paperQuote:
        "We take an idea and return a finished brand, book, store, film, or system — one coordinated engagement, not a freelance roster.",
      stats: [
        { k: "16", v: "Practices under one roof" },
        { k: "5", v: "Launch packages you can buy now" },
        { k: "3", v: "Ways to engage, from one task to end-to-end" },
      ],
      ...
      servicesDek:
        "The public site shows departments. Detailed services appear after you choose a practice — or after you tell reception the outcome.",
      studioKicker: "The studio",
      studioTitle: "One coordinated team — not a freelance roster.",
      studioBody1:
        "Clients hire us for one task, a connected package, or a complete start-to-finish solution. AI is the operating infrastructure for reception, intake, routing, and coordination. It is not a replacement for professional judgment, licensed advice, or human accountability.",
      studioBody2:
        "You own every account. We do not create marketplace or banking profiles in the agency’s name, and we do not hold passwords when an invitation will do.",
    ...
    steps: PUBLIC_STEPS.map((s) => ({ n: s.n, title: s.title, body: s.body })),
    ...
    legal: {
      ...
      terms: TERMS_SECTIONS.map((s) => ({ heading: s.heading, paragraphs: [...s.paragraphs] })),
      privacy: PRIVACY_SECTIONS.map((s) => ({ heading: s.heading, paragraphs: [...s.paragraphs] })),
      allows: [...LEGAL_ALLOWS],
...
function mergeHero(base: HeroCopy, patch: Partial<HeroCopy> | undefined): HeroCopy {
  return {
...
function mergeLines(value: unknown, fallback: string[]): string[] {
  if (!Array.isArray(value) || value.length === 0) return fallback;
  return value.map((item) => (typeof item === "string" ? item : "")).filter((item) => item.trim());
}

function mergeSections(value: unknown, fallback: LegalSection[]): LegalSection[] {
  if (!Array.isArray(value) || value.length === 0) return fallback;
  return value
    .map((item) => {
      const row = item && typeof item === "object" ? (item as LegalSection) : { heading: "", paragraphs: [] };
      return {
...
export function mergeSiteCopy(raw: unknown): SiteCopy {
  ...
  const saved = raw && typeof raw === "object" ? (raw as Partial<SiteCopy>) : {};
  const home = { ...d.home, ...(saved.home ?? {}) };
  const stats =
    Array.isArray(saved.home?.stats) && saved.home.stats.length
      ? saved.home.stats.map((s, i) => ({
          k: str(s?.k, d.home.stats[i]?.k ?? ""),
          v: str(s?.v, d.home.stats[i]?.v ?? ""),
        }))
      : d.home.stats;
  const faqs =
    Array.isArray(saved.faqs) && saved.faqs.length
      ? saved.faqs
  ...
  const steps =
    Array.isArray(saved.steps) && saved.steps.length
      ...
      : d.steps;
  const engagements =
    Array.isArray(saved.engagements) && saved.engagements.length
      ? saved.engagements.map((e, i) => ({
  ...
  return {
    home: { ...mergeHero(d.home, home), ...home, stats },
