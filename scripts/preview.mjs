...
async function restart() {
  ...
  // Nitro bundles PGLite's JS but not pglite.data / pglite.wasm. Local preview
  // has no DATABASE_URL so it uses PGLite — copy the assets next to the chunk.
  const pgliteLibs = join(ROOT, ".vercel/output/functions/__server.func/_libs");
  const pgliteDist = join(ROOT, "node_modules/@electric-sql/pglite/dist");
  if (existsSync(pgliteLibs) && existsSync(pgliteDist)) {
    for (const name of ["pglite.data", "pglite.wasm"]) {
      const from = join(pgliteDist, name);
      const to = join(pgliteLibs, name);
